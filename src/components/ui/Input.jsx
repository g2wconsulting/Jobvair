...
export function Input({ label, id, hint, error, required = false, className = "", ...props }) {
  ...
  return (
    <label className={["jv-field", className].filter(Boolean).join(" ")} htmlFor={inputId}>
      ...
      <input id={inputId} className="jv-input" aria-invalid={Boolean(error)} required={required} {...props} />
