  ...
  const saveResume = async () => {
    ...
    try {
      ...
      if (rid) {
        ...
        const { data } = await supabase.from("resumes").insert({ user_id:user.id, name:resumeName, template:selectedTmpl?.slug||"modern", selected_template_id:selectedTmpl?.id||null, is_primary:false, contact_fields:currentHeaderConfig, sections:[] }).select().single();
      ...
      const allSections = (sections || []).map(s => ({
        ...
        content:      s.section_type === "name" ? currentHeaderConfig : (s.content || {}),
...
// ── App Shell ─────────────────────────────────────────────────────────────
>>>>>>> fb4b75e (Fix resume builder header persistence)
